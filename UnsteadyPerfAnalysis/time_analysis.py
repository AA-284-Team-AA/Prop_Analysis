...
def plot_results(t, R, mdot_ox, mdot_fuel, Pc, thrust, ISP, filename):
    ...
    ax[i].set_ylabel(r'$G_\mathrm{ox}$ [\SI{}{kg/m^2/s}]')
    ax[i].set_ylim(0, 200.)
    i += 1
...
def plot_results_separate(t, R, mdot_ox, mdot_fuel, Pc, thrust):
    fig, ax = plt.subplots()
    ax.plot(t, mdot_ox)
    ax.set_ylabel(r'$\dot{m}_\mathrm{ox}$ [$\SI{}{kg/m^3}$]')
    fig.savefig('mdotox.'+FORMAT, format=FORMAT)

    fig, ax = plt.subplots()
    ax.plot(t, mdot_fuel)
    ax.set_ylabel(r'$\dot{m}_\mathrm{fuel}$ [$\SI{}{kg/m^3}$]')
    ...
    ax.set_ylabel(r'$D_\mathrm{inner}$ [in]')
    ...
    ax.set_ylabel(r'$G_\mathrm{ox}$ [\SI{}{kg/m^2/s}]')
...
def print_ranges(t, R, mdot_ox, mdot_fuel, Pc, thrust):
    ...
    print('mdotox: %f - %f' % (np.min(mdot_ox), np.max(mdot_ox)))
    print('Pc: %f - %f' % (np.min(Pc)/PSI_TO_PA, np.max(Pc)/PSI_TO_PA))
    print('OF: %f - %f' % (np.min(mdot_ox/mdot_fuel),
...
if __name__ == '__main__':
    ...
    print_ranges(t, R, mdot_fuel, mdot_ox, Pc, thrust)
