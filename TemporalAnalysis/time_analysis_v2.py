...
def plot_results(t, R, mdot_ox, mdot_fuel, Pc, thrust, ISP, filename):
    ...
    ax[i].plot(t, mdot_ox) # mdot ox plot
    ax[i].set_ylabel(r'$\dot{m}_{ox}$ [${kg/m^3}$]')
    i += 1

    ax[i].plot(t, mdot_fuel) # mdot fuel plot
    ax[i].set_ylabel(r'$\dot{m}_{fuel}$ [${kg/m^3}$]')
    i += 1

    ax[i].plot(t, mdot_ox/mdot_fuel) # O/F ratio plot
    ax[i].set_ylabel(r'O/F')
    i += 1

    ax[i].plot(t, Pc / PSI_TO_PA) # chamber pressure plot
    ax[i].set_ylabel(r'$P_c$ $[{psi}]$')
    i += 1

    ax[i].plot(t, thrust) # Thrust plot
    ax[i].set_ylabel(r'$T$ [N]')
    i += 1

    ax[i].plot(t, 2*R*M_TO_IN) # inner diameter plot (fuel regression)
    ax[i].set_ylabel(r'$D_{inner}$ [in]')
    i += 1

    ax[i].plot(t, Pe) # exit nozzle pressure plot
    ax[i].set_ylabel(r'$P_e$ [Pa]')
    i += 1

    ax[i].plot(t, mdot_ox / (np.pi * R**2)) # oxidizer mass flux
    ax[i].set_ylabel(r'$G_{ox}$ [{kg/m^2/s}]')
    ax[i].set_ylim(0, 200.)
    i += 1

    ax[i].plot(t, ISP) # Isp plot
